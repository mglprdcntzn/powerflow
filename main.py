...
for subset in subsets_algorithms:
    ...
    ax1.plot(Nnodos,meantime[:,indices], marker='*', linestyle='-', markersize=5)
    ax1.grid(True)
    ax1.legend(subset,loc='upper left', fontsize=font_size-2)
    ax1.set_xticks(Nnodos[1::2])
    ax1.set_title('Mean iteration time', fontsize=font_size)
    ax1.set_ylabel('[ms]', fontsize=font_size)
    ...
    plt.savefig('time_'+file_name_sfx, format='eps', bbox_inches="tight")
    plt.show()
    plt.close()
    ############################################
    #iterations vs nodes
    fig, ax2 = plt.subplots(figsize=fig_size)
    ax2.plot(Nnodos,meaniterations[:,indices], marker='*', linestyle='-', markersize=5)
    ax2.grid(True)
    # ax2.legend(subset,loc='upper left', fontsize=font_size-2)
    ax2.set_xticks(Nnodos[1::2])
    ax2.set_title('Mean iterations', fontsize=font_size)
    ax2.set_ylabel(' ', fontsize=font_size)
    ax2.set_xlabel('N', fontsize=font_size)
    ax2.tick_params(axis='both', labelsize=font_size)
    
    plt.subplots_adjust(left=left_space,right=right_space, top=top_space, bottom=bottom_space)
    plt.savefig('iterations_'+file_name_sfx, format='eps', bbox_inches="tight")
    plt.show()
    plt.close()
    ############################################
    #gues vs nodes
    fig, ax3 = plt.subplots(figsize=fig_size)
    ax3.plot(Nnodos,meanabsgues[:,indices], marker='*', linestyle='-', markersize=5)
    ax3.grid(True)
    ax3.legend(subset,loc='upper left', fontsize=font_size-2)
    ax3.set_xticks(Nnodos[1::2])
    ax3.set_title('Mean ||g(.)||', fontsize=font_size)
    ax3.set_ylabel('[p.u.]', fontsize=font_size)
    ax3.set_xlabel('N', fontsize=font_size)
    ...
    fig, ax4 = plt.subplots(figsize=fig_size)
    ax4.plot(Nnodos,meanerror[:,indices], marker='*', linestyle='-', markersize=5)
    ax4.grid(True)
    ax4.legend(subset,loc='upper left', fontsize=font_size-2)
    ax4.set_xticks(Nnodos[1::2])
    ax4.set_title('Mean voltage error w/r to '+algorithms[0], fontsize=font_size)
    ax4.set_xlabel('N', fontsize=font_size)
    ax4.set_ylabel('%', fontsize=font_size)
    ...
    ax5.plot(Nnodos,meantime[:,indices[1:]]/(meantime[:,indices[0]].reshape(-1,1)@np.ones((1,len(subset)-1))), marker='*', linestyle='-', markersize=5)
    ax5.grid(True)
    ax5.legend(subset[1:],loc='upper left', fontsize=font_size-2)
    ax5.set_xticks(Nnodos[1::2])
    ax5.set_title('Time rate w/r to '+subset[0], fontsize=font_size)
    ax5.set_ylabel('[ms/ms]', fontsize=font_size)
    ax5.set_xlabel('N', fontsize=font_size)
    ax5.tick_params(axis='both', labelsize=font_size)
    
    plt.subplots_adjust(left=left_space,right=right_space, top=top_space, bottom=bottom_space)
    plt.savefig('timerate_'+file_name_sfx, format='eps', bbox_inches="tight")
    plt.show()
    plt.close()
    ############################################